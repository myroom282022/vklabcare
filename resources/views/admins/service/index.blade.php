                    ...
                    <tbody>
                        ...
                        @foreach($slider as $data)
                            <tr>
                                <td>{{$useId}}</td>
                                <td>
                                <img src="{{asset('public/storage/service/img/'.$data->image_name)}}" class="img-fluid" alt="user" style="height: 40px; width:40px;  border-radius: 50%;">
                                </td>
                                <td>{{$data->title}}</td>
                                <td>{{$data->description}}</td>
                                <td>
                                    <a href="{{asset('admin/service/edit/'.$data->id)}}"   id="editbutton"><i class="fas fa-edit text-warning" aria-hidden="true" style="font-size:18px" data-bs-toggle="modal" data-bs-target="#edituser"></i></a>
...
<script>
  ...
  $(".delete_confirm").on('click',function(e){
    ...
    swal({
            ...
            if (willDelete) {
                var url = "{{url('admin/service/delete')}}/"+ delete_id;
