                ...
                <tbody>
                    ...
                    @foreach($slider as $data)
                        <tr>
                            <td>{{$useId}}</td>
                            <td>
                            <img src="{{url('storage/slider/img/'.$data->slider_image)}}" class="img-fluid" alt="user" style="height: 40px; width:40px;  border-radius: 50%;">

                            </td>
                            <td><textarea cols="num" rows="num">{{$data->title}}</textarea></td>
                            <td><textarea cols="num" rows="num">{{$data->description}}</textarea></td>
                            <td>

                                <a href="{{url('admin/slider/edit/'.$data->id)}}"   id="editbutton"><i class="fas fa-edit text-warning" aria-hidden="true" style="font-size:18px" data-bs-toggle="modal" data-bs-target="#edituser"></i></a>
